...
var slot_script = preload("./equipmentSlot.gd")
...
func _on_player_init(player):
	player.equipment.connect("inventory_changed", self, "_on_player_inventory_changed")
...
func slots_gui_input(event: InputEvent, slot: equipment_slot):
